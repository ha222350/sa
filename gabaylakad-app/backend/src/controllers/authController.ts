...
export const register = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const deviceRes = await query('SELECT * FROM device WHERE serial_number = ?', [device_id]);
    ...
    const isActive = device.is_active === 1 || device.is_active === true || (Buffer.isBuffer(device.is_active) && device.is_active[0] === 1);
        ...
        await query('INSERT INTO user (name, first_name, last_name, email, phone_number, impairment_level, device_id, password, is_verified, verification_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [name, firstName, lastName, email, phone_number, impairment_level, device_id, hashedPassword, false, verificationCode]);
